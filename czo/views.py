...
def kritsnam(request):
    return render(request,'czo/index1.html')
...
def set_pass(request):
    ...
    if request.method == "POST":
        ...
        if u.first_name != '':
            ...
            return render(request,'czo/set_pass.html', {'error_message': 'Wrong Username'}  )
    else:
        print('lol_get')
    ...
    if request.method == "POST":
        ...
        if user:
            ...
            message =   ("Hi " + username + " Put this number in form to verify your Email : "
                        + str(x) + ", you can also follow to this link to set new_pass to your Account http://127.0.0.1:8000/czo/set_pass/"
                        + username)
            to_email = user.email
        ...
        if request.method == "POST":
            ...
            if len(myuser.last_name) < 5:
                ...
                message =   ("Hi " + myuser.username + " Put this number in form to verify your Email : "
                            + str(x) + ", you can also follow to this link to verify your otp http://127.0.0.1:8000/czo/otp/"
                            + myuser.username)
            ...
            if (mynodes):
                ...
                if (sensors):
                    ...
                    if node!={}:
                        timedelta1 = timedelta(days=7)
                        datas = Data.objects.filter(sensor_name=sensors[0].id, doc__gte= datetime.datetime.now()-timedelta1)
                        return render(request, 'czo/sensors.html' , { 'mynodes' : mynodes, 'datas' : datas , 'sensor_id' : sensor_id})
                ...
                if (mynodes):
                    ...
                    if (sensors):
                        ...
                        if node!={}:
                            ...
                            datas = Data.objects.filter(sensor_name=sensors[0].id, doc__gte= datetime.datetime.now()-timedelta1)
                            name = sensors[0].name
                            return render(request, 'czo/sensor.html' , { 'mynodes' : mynodes, 'datas' : datas , 'nodeName' : node_req,'sensor_id' :sensor_id})
